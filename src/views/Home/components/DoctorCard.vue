...
const onFollowById = async (item: Doctor) => {
  await followDoctorAPI(props.item.id)
  showSuccessToast(props.item.likeFlag ? '取消关注' : '关注成功')
  // doc.likeFlag = doc.likeFlag === 1 ? 0 : 1
  emit('followById', item.id)
}
// 关注逻辑
// const onFollowById = async (item: Doctor) => {
